...
def _setup_process_change_mocks(orchestrator, intent_type: IntentType):
    ...
    orchestrator._dispatch_explore = AsyncMock(return_value=True)
    orchestrator._dispatch_diagnoser = AsyncMock(return_value=True)
    orchestrator._dispatch_review = AsyncMock(return_value=True)
    orchestrator._run_phases = AsyncMock(return_value=True)

    return p_classify, p_route, p_read


def test_research_routes_to_dispatch_explore(orchestrator):
    ...
    with p_c, p_r, p_rd:
        _run(orchestrator._process_change(_make_prop()))

    orchestrator._dispatch_explore.assert_called_once()
    orchestrator._run_phases.assert_not_called()


def test_implementation_routes_to_pipeline(orchestrator):
...
def test_fix_routes_to_pipeline_with_skip_enrich(orchestrator):
...
def test_investigation_routes_to_dispatch_diagnoser(orchestrator):
...
def test_evaluation_routes_to_dispatch_review(orchestrator):
...
def test_open_ended_returns_false_without_dispatching(orchestrator):
    """OPEN_ENDED intent → returns False, no dispatch method called."""
    p_c, p_r, p_rd = _setup_process_change_mocks(
        orchestrator, IntentType.OPEN_ENDED
    )
    with p_c, p_r, p_rd:
        ret = _run(orchestrator._process_change(_make_prop()))
