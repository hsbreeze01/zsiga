...
## Summary
为无测试模块 `zsiga/config.py` (496 行, 4 函数, 13 类) 添加单元测试覆盖。
...
- 函数数: 4，类数: 13
- ruff lint 问题: 0
- 圈复杂度: 平均 3.3，高 CC(>10) 函数 1 个

### 函数列表
- `_find_config()` L9-L17 (~9L)
- `_resolve_env_vars(value)` L20-L28 (~9L)
- `validate_config(config)` L284-L333 (~50L)
- `load_config(path)` L336-L495 (~160L)

### 类结构
- `ValidationResult` L32-L38 methods=['valid']
- `ConfigValidationError` L41-L44 methods=['__init__']
- `SSHConfig` L47-L53 methods=['__init__']
- `TargetConfig` L56-L70 methods=['__init__']
- `LLMConfig` L73-L83 methods=['__init__']

### Lint 问题
- 无 lint 问题

### 高复杂度函数 (CC > 10)
- `validate_config` L284 CC=17 (50L)
