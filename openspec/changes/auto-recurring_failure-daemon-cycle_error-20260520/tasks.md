...
- [ ] **1.1** Add per-proposal try/except in `ZsigaOrchestrator.run_cycle()` (in `zsiga/pipeline/orchestrator.py`)
...
- [ ] **2.1** Move `ZsigaOrchestrator(config)` inside the try/except in `daemon_loop()` (in `zsiga/daemon.py`)
...
- [ ] **2.2** Enrich error diagnostics in daemon_loop exception handler (in `zsiga/daemon.py`)
...
- [ ] **3.1** Add test file `tests/test_daemon_cycle_resilience.py` with tests for:
